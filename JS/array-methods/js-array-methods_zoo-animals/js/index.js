...
// notes:
//check for evry animal in animals if it is included
//if  yes return true
//if not return false

function hasAnimal(animals, animalName) {
  const animalsLowerCase = animals.map((animal) => {
    return animal.toLowerCase();
  });
  return animalsLowerCase.includes(animalName.toLowerCase()) ? true : false;
